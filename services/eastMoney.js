/**
 * 东方财富 / 新浪财经 行情与财务数据封装（Node 服务端）
 * 前端只调本后端接口，参数与主机兜底在此处理。
 */
...
/**
 * 解析 holdings：支持数组对象 / secids 字符串 / JSON 字符串
 * @returns {Array<{ code: string, market?: number, name?: string }>}
 */
function parseHoldings(input) {
      ...
      .map((h) => {
        if (typeof h === 'string') {
          ...
          if (parts.length === 2 && /^\d+$/.test(parts[0])) {
            return { market: Number(parts[0]), code: parts[1] };
          }
          return { code: h };
        }
        if (!h || h.code == null) return null;
        return {
          code: String(h.code).trim(),
          market: h.market != null ? Number(h.market) : undefined,
          name: h.name
        };
      })
      .filter((h) => h && h.code);
  ...
  if (typeof input === 'string') {
    ...
    if (trimmed.startsWith('[')) {
      try {
        return parseHoldings(JSON.parse(trimmed));
      } catch (_) {
        /* fallthrough */
      }
    }
    return parseHoldings(trimmed.split(',').map((s) => s.trim()).filter(Boolean));
  }
  return [];
}
...
/**
 * 新浪财经实时报价（服务端 HTTP + GBK 解码）
 * @returns {Promise<Object>} code -> { name, price, change }
 */
async function loadSinaQuotes(holdings, timeoutMs = 5000) {
  ...
  try {
    const resp = await fetch(
      'https://hq.sinajs.cn/list=' + list + '&_=' + Date.now(),
      {
        headers: {
    ...
    if (!resp.ok) throw new Error('行情接口加载失败');
    ...
    if (err.name === 'AbortError') {
      throw new Error('行情请求超时，请检查网络后重试');
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
function normalizeHolderAction(row) {
  ...
  const changeNum =
    typeof raw === 'number'
      ? raw
      : numOrNull(String(raw || '').replace(/,/g, ''));
...
async function loadStockProfile(holding) {
  ...
  if (marketKind === 'CN') {
    ...
    financePromise = loadOverseasFinancialReports(primary, 'US').catch(
      (err) => {
...
async function loadCnSectorStocks(boardCodeOrCodes, limit = 20, kind = 'gainers') {
  const codes = (
    Array.isArray(boardCodeOrCodes)
      ? boardCodeOrCodes
  ...
  const perBoard =
    codes.length === 1 ? take : Math.min(50, Math.max(take, 30));
